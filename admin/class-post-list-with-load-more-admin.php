...
class Post_List_With_Load_More_Admin {
	...
	public function plugin_settings_page_content() {        ?>
		...
		<div class="wrap">
			...
			$active_tab       = 'layout';
			if ( wp_verify_nonce( sanitize_text_field( wp_unslash( isset( $_REQUEST['_wpnonce'] ) ? $_REQUEST['_wpnonce'] : '' ) ), 'tab_action_nonce' ) ) {
				if ( isset( $_GET['tab'] ) ) {
			...
			<h2 class="nav-tab-wrapper">
				<a href="?page=post_list_with_load_more_settings&tab=layout&_wpnonce=<?php echo esc_attr( $tab_action_nonce ); ?>" class="nav-tab <?php echo 'layout' === $active_tab ? 'nav-tab-active' : ''; ?>">Settings</a>
				<a href="?page=post_list_with_load_more_settings&tab=shortcode&_wpnonce=<?php echo esc_attr( $tab_action_nonce ); ?>" class="nav-tab <?php echo 'shortcode' === $active_tab ? 'nav-tab-active' : ''; ?>">Generate Shortcode</a>
			</h2>
